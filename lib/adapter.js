...
const Adapter = {
  ...
  registerConnection (connection, collections, cb) {
    ...
    // Where are we writing the DB to?
    let filename = connection.filename || connection.path + '/' + connection.identity + '.sqlite'

    // Check if it's an in memory instance.
    ...
    const filePath = filename !== ":memory:" ? path.dirname(filename) : false
    ...
    this.connections.set(connection.identity, {
      ...
      knex: Knex({
        client: 'sqlite3',
        connection: {
          filename: filename + (filename.endsWith('.sqlite') ? '' : '.sqlite')
        },
  ...
  find (connectionName, tableName, options, cb) {
    ...
    // Remove any empty where clauses, they seem to cause
    // issues.
    // @link https://github.com/trailsjs/trailpack-waterline/issues/25
    // @link https://github.com/waterlinejs/sqlite3-adapter/issues/1#issuecomment-145383650
    if (options.where) {
      Object.keys(options.where).forEach(column => {
        if (Array.isArray(options.where[column]) && options.where[column].length === 0)
          delete options.where[column]
      })
